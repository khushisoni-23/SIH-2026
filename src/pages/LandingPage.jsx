...
export const LandingPage = () => {
  ...
  const flowSteps = [
    { icon: Database, color: "#06b6d4", title: "1. Data Ingestion", sub: "Baltic Dry Index, AIS live feeds, Platts prices, NOAA weather & tidal tables." },
    { icon: TrendingUp, color: "#3b82f6", title: "2. AI Freight Forecast", sub: "Neural networks forecast 30/60/90-day spot and contract rate directions." },
    { icon: Anchor, color: "#10b981", title: "3. Port & Vessel Match", sub: "Dynamic draft clearance check, berth congestion & DWT compatibility." },
    { icon: Shield, color: "#8b5cf6", title: "4. Risk & Route Sentinel", sub: "Cyclone tracking, bunker consumption simulation & CII carbon score." },
    { icon: FileCheck2, color: "#f59e0b", title: "5. Smart Charter Decision", sub: "Optimal Spot vs. COA allocation recommendation with laytime audit." },
  ];

  const features = [
    { icon: BarChart3, color: "#06b6d4", title: "Freight Forecasting Engine", desc: "Predict freight rate trends 30, 60, and 90 days ahead with confidence intervals across global bulk corridors.", link: "/forecast" },
    { icon: Ship, color: "#3b82f6", title: "Vessel Recommendation", desc: "Automate vessel class and laycan matching against cargo parcel sizes and terminal draft constraints.", link: "/vessels" },
    { icon: Anchor, color: "#10b981", title: "Port Intelligence", desc: "Live tidal draft predictions, berth waiting times, queue length, and crane throughput for East Coast India.", link: "/ports" },
    { icon: Navigation, color: "#8b5cf6", title: "Route & Bunker Simulator", desc: "Compare Malacca vs Sunda vs Cape routes for voyage time, VLSFO fuel burn, and carbon compliance.", link: "/routes" },
    { icon: AlertTriangle, color: "#ef4444", title: "Risk & Chokepoint Alerts", desc: "Real-time alerts on tropical cyclones, bunker price volatility, geopolitical choke points, and port strikes.", link: "/risk" },
    { icon: FileCheck2, color: "#f59e0b", title: "Charter Planning & COA", desc: "Spot vs. COA allocation optimizer, laytime calculator, and demurrage dispute audit tools.", link: "/charter" },
  ];

  const faqs = [
    { q: "How does FreightSense predict maritime freight rates with 94.2% accuracy?", a: "FreightSense uses a hybrid machine learning pipeline (LSTM + Transformer neural networks) trained on 15+ years of Baltic Dry Index (BDI), Platts bulk freight assessments, historical Singapore/Fujairah bunker prices, global vessel fleet supply, and steel-mill import cycles." },
    { q: "How does the platform verify vessel draft compatibility with Indian ports?", a: "The Port Intelligence engine continuously cross-references real-time tidal tables, channel dredge depths, and seasonal siltation data for Paradip, Haldia, Visakhapatnam, Dhamra, and Ennore with vessel loaded drafts to prevent grounding risks and tidal delay demurrage." },
    { q: "How does the platform advise on Spot vs. Long-Term (COA) Chartering?", a: "When the freight forecast predicts rate volatility exceeding 8% or an upward price trend, the system advises locking in 3 to 6-month Contracts of Affreightment (COA). When rates are forecasted to soften, it recommends spot chartering to capture lower market rates." },
    { q: "Is FreightSense tailored for SAIL and Ministry of Steel requirements?", a: "Yes. The platform is architected specifically for raw-material procurement (coking coal, iron ore, limestone, dolomite) feeding SAIL's integrated steel plants at Bhilai, Rourkela, Bokaro, Durgapur, and IISCO Burnpur." },
    { q: "Can we simulate voyage costs and bunker consumption across alternate sea routes?", a: "Yes. The Route Analysis module allows charterers to simulate routing options (e.g. via Malacca Strait vs. Sunda Strait vs. Cape of Good Hope) with accurate VLSFO fuel consumption, weather routing, and CII carbon ratings." }
  ];
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-300 ${isLight ? "bg-[#f4f7fb] text-slate-900" : "bg-[#030712] text-slate-100"}`}>
      ...
      {/* STICKY TOP NAVBAR WITH DIRECT MODULE NAVIGATION */}
      <nav className={`sticky top-0 z-50 h-16 px-6 lg:px-12 flex items-center justify-between gap-4 backdrop-blur-xl border-b transition-all ${isLight ? "bg-white/95 border-slate-200/90" : "bg-[#030712]/90 border-slate-800/80"}`}>
        <Link to="/" className="flex items-center gap-3 no-underline group shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className={`font-black text-base tracking-tight ${isLight ? "text-slate-900" : "text-white"}`}>FreightSense</span>
              <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded border uppercase tracking-wider ${isLight ? "bg-cyan-50 text-cyan-700 border-cyan-300" : "bg-cyan-500/10 text-cyan-300 border-cyan-500/30"}`}>
                Enterprise AI
              </span>
            </div>
            <span className={`block text-[10px] font-semibold uppercase tracking-wider ${isLight ? "text-cyan-700" : "text-cyan-400"}`}>
        ...
        <div className="hidden xl:flex items-center gap-6 text-xs font-semibold">
          {[
            ...
            { label: "Freight Forecast", href: "/forecast" },
            { label: "Vessel Optimization", href: "/vessels" },
            { label: "Port Intelligence", href: "/ports" },
            { label: "Route Simulator", href: "/routes" },
            { label: "Charter Planning", href: "/charter" },
            { label: "Market Data", href: "/market-data" },
            { label: "Risk Sentinel", href: "/risk" }
          ].map(l => (
            <Link 
              key={l.href} 
              to={l.href} 
              className={`no-underline transition-colors ${isLight ? "text-slate-600 hover:text-cyan-600" : "text-slate-300 hover:text-cyan-400"}`}
            >
              {l.label}
            </Link>
          ))}
        ...
        {/* Right Action: Language Switcher, Theme Toggle, & Login Button */}
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          
          {/* Language Switcher */}
          <div className={`flex rounded-xl overflow-hidden border text-[11px] font-bold ${isLight ? "border-slate-300" : "border-slate-700"}`}>
            <button 
              onClick={() => changeLanguage("en")} 
              className={`px-2.5 py-1 transition-colors ${language === "en" ? "bg-cyan-600 text-white" : isLight ? "text-slate-600 hover:bg-slate-100" : "text-slate-400 hover:bg-slate-800"}`}
            >
              EN
            </button>
            <button 
              onClick={() => changeLanguage("hi")} 
              className={`px-2.5 py-1 transition-colors ${language === "hi" ? "bg-cyan-600 text-white" : isLight ? "text-slate-600 hover:bg-slate-100" : "text-slate-400 hover:bg-slate-800"}`}
              style={{ borderLeft: isLight ? "1px solid #cbd5e1" : "1px solid rgba(255,255,255,0.1)" }}
            >
              हिन्दी
            </button>
          </div>
          ...
          <button 
            ...
            className={`p-2 rounded-xl border transition-colors ${isLight ? "border-slate-300 text-slate-700 hover:bg-slate-100" : "border-slate-700 text-slate-300 hover:bg-slate-800"}`}
          >
            {isLight ? <Moon size={14} /> : <Sun size={14} className="text-amber-400" />}
          </button>

          {/* Login Button (Replaced Launch Dashboard per requirement) */}
          <button 
            onClick={() => navigate("/login")} 
            className="px-5 py-2 text-xs font-bold bg-gradient-to-r from-cyan-500 via-cyan-600 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white rounded-xl shadow-lg shadow-cyan-600/25 flex items-center gap-2 transition-all hover:scale-[1.02] active:scale-95"
          >
            <LogIn size={14} />
            <span>Login</span>
          </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center py-14 lg:py-20">
            ...
            <div className="lg:col-span-6 xl:col-span-7 flex flex-col gap-6">
              <div className={`inline-flex items-center gap-2 self-start px-3.5 py-1.5 rounded-full text-xs font-bold border ${isLight ? "bg-cyan-50 text-cyan-800 border-cyan-200" : "bg-cyan-500/10 text-cyan-300 border-cyan-500/25"}`}>
                ...
                <span>SIH 2026 Innovation — Ministry of Steel &amp; SAIL Logistics</span>
              </div>
              ...
              <h1 className={`text-3xl sm:text-4xl lg:text-5xl font-black leading-tight tracking-tight ${isLight ? "text-slate-900" : "text-white"}`}>
                ...
                <span className="bg-gradient-to-r from-cyan-500 via-teal-400 to-blue-500 bg-clip-text text-transparent">
                  Optimized Vessel Chartering
                </span>
              </h1>

              <p className={`text-sm sm:text-base leading-relaxed max-w-xl ${isLight ? "text-slate-600" : "text-slate-300"}`}>
                AI-driven freight rate forecasting, real-time East Coast port draft sentinel, and automated Spot vs. COA allocation for bulk coking coal &amp; iron ore procurement.
              </p>
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                ...
                >
                  <LogIn size={16} />
                  <span>Login / Access Platform</span>
                  <ArrowRight size={15} />
                </button>
                <a 
                  href="#simulator" 
                  className={`px-6 py-3.5 text-sm font-semibold rounded-xl border flex items-center gap-2 transition-all ${isLight ? "bg-white text-slate-800 border-slate-300 hover:border-cyan-500 shadow-sm" : "bg-slate-900/90 text-slate-200 border-slate-700 hover:border-cyan-500 hover:bg-slate-800"}`}
                >
                  <Compass size={16} className="text-cyan-400" />
                  <span>Test Voyage Simulator</span>
                </a>
              </div>
              ...
              <div className={`grid grid-cols-3 gap-4 pt-6 border-t ${isLight ? "border-slate-200" : "border-slate-800"}`}>
                <div>
                  ...
                  <div className={`text-xs font-semibold mt-0.5 ${isLight ? "text-slate-500" : "text-slate-400"}`}>Annual SAIL Bulk Cargo</div>
                </div>
                <div>
                  <div className="text-xl sm:text-2xl font-black text-cyan-500">38.6% Cut</div>
                  <div className={`text-xs font-semibold mt-0.5 ${isLight ? "text-slate-500" : "text-slate-400"}`}>Demurrage Costs</div>
                </div>
                <div>
                  <div className="text-xl sm:text-2xl font-black text-emerald-500">$182K+</div>
                  <div className={`text-xs font-semibold mt-0.5 ${isLight ? "text-slate-500" : "text-slate-400"}`}>Avg. Savings / Voyage</div>
                </div>
              </div>
            </div>

            {/* Right Column: Clean Cinematic Vessel Showcase (Uncluttered Image) */}
            <div className="lg:col-span-6 xl:col-span-5 relative">
              <div className={`rounded-3xl overflow-hidden border shadow-2xl transition-all duration-300 hover:shadow-cyan-500/10 ${isLight ? "border-slate-200 bg-white" : "border-slate-800 bg-slate-900"}`}>
                ...
                {/* Cinematic Image Frame without blocking overlays */}
                <div className="relative h-80 sm:h-96 lg:h-[400px] bg-slate-950 img-zoom-container group">
                  <img 
                  ...
                  <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between text-white text-xs font-bold">
                    <div className="flex items-center gap-2">
                      ...
                      <span>MV Vishva Vijay · Capesize (180,000 DWT)</span>
                    </div>
                    <span className="text-[11px] font-mono text-cyan-300">Port Hedland → Paradip</span>
                  </div>
                </div>
                ...
                <div className={`p-4 grid grid-cols-3 gap-3 text-center text-xs border-t ${isLight ? "bg-slate-50 text-slate-800 border-slate-200" : "bg-slate-950 text-slate-200 border-slate-800"}`}>
                  <div>
                    <div className="text-[10px] text-slate-500 uppercase font-bold">Loaded Draft</div>
                    <div className="text-xs font-black text-emerald-500 mt-0.5">17.2m Safe</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-slate-500 uppercase font-bold">Optimal Window</div>
          ...
          <div className="flex items-center gap-6 overflow-x-auto whitespace-nowrap flex-1 justify-start md:justify-end text-xs" style={{ scrollbarWidth: "none" }}>
            {tickers.map((t, i) => (
              <div key={i} className="flex items-center gap-1.5 shrink-0">
                <span className={isLight ? "text-slate-500 font-medium" : "text-slate-400 font-medium"}>{t.label}:</span>
                <span className={`font-bold ${isLight ? "text-slate-900" : "text-white"}`}>{t.value}</span>
                {t.change && (
                  <span className={`text-[10px] font-bold px-1.5 py-0.2 rounded ${t.up ? (isLight ? "bg-emerald-50 text-emerald-700" : "bg-emerald-500/15 text-emerald-400") : (isLight ? "bg-rose-50 text-rose-700" : "bg-rose-500/15 text-rose-400")}`}>
                    {t.change}
                  </span>
                )}
              </div>
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          <div className="text-center max-w-2xl mx-auto mb-12 space-y-3">
            ...
            <h2 className={`text-2xl sm:text-3xl font-black ${isLight ? "text-slate-900" : "text-white"}`}>
              Simulate Bulk Procurement &amp; Charter Strategy
            </h2>
            <p className={`text-sm ${isLight ? "text-slate-600" : "text-slate-400"}`}>
              Select origin port, destination steel plant terminal, and cargo parameters to view instant AI rate forecasting and optimal charter allocation.
            </p>
          </div>
              ...
              <div className="lg:col-span-7 space-y-5">
                ...
                <div className={`p-4 rounded-xl flex items-center justify-between text-xs ${isLight ? "bg-blue-50 text-blue-900 border border-blue-200" : "bg-blue-950/30 text-blue-200 border border-blue-900/50"}`}>
                  <div className="flex items-center gap-2">
                    ...
                    <span>Selected route: <strong>{currentSim.dist.toLocaleString()} Nautical Miles</strong> via standard deep-water passage.</span>
                  </div>
                  <span className="font-bold text-cyan-600 dark:text-cyan-400">~{currentSim.days} Days Transit</span>
                </div>
              </div>
              ...
              <div className="lg:col-span-5">
                <div className={`rounded-2xl p-6 border space-y-4 ${isLight ? "bg-gradient-to-br from-cyan-50/70 to-blue-50/70 border-cyan-200" : "bg-gradient-to-br from-cyan-950/40 via-slate-900 to-blue-950/40 border-cyan-800/60"}`}>
                  ...
                  <div className="space-y-1">
                    <div className="text-xs text-slate-500 dark:text-slate-400">Forecasted Freight Rate</div>
                    <div className="flex items-baseline gap-2">
                      <span className={`text-3xl font-black ${isLight ? "text-slate-900" : "text-white"}`}>
                        ${estimatedRate.toFixed(2)}
                      </span>
                      <span className="text-xs font-bold text-slate-500">/ Metric Tonne</span>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-3 pt-2">
                    <div className={`p-3 rounded-xl border ${isLight ? "bg-white border-slate-200" : "bg-slate-900/90 border-slate-800"}`}>
                      <div className="text-[10px] text-slate-500 uppercase font-bold">Charter Strategy</div>
                      <div className="text-xs font-black text-cyan-500 mt-0.5">{currentSim.strategy}</div>
                    </div>
                    <div className={`p-3 rounded-xl border ${isLight ? "bg-white border-slate-200" : "bg-slate-900/90 border-slate-800"}`}>
                      <div className="text-[10px] text-slate-500 uppercase font-bold">Est. Savings</div>
                  ...
                  >
                    <span>Open in Charter Planning Platform</span>
                    <ArrowRight size={13} />
                  </button>
                </div>
              </div>
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          <div className="text-center max-w-2xl mx-auto mb-14 space-y-3">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${isLight ? "bg-cyan-50 text-cyan-800 border-cyan-200" : "bg-cyan-500/10 text-cyan-300 border-cyan-500/25"}`}>
              ...
              <span>Full Decision Suite</span>
            </span>
            <h2 className={`text-2xl sm:text-3xl font-black ${isLight ? "text-slate-900" : "text-white"}`}>
              Six Specialized Intelligence Engines
            </h2>
            <p className={`text-sm ${isLight ? "text-slate-600" : "text-slate-400"}`}>
              Engineered to resolve every complex decision in overseas bulk cargo chartering and terminal logistics.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((f) => {
              ...
              return (
                <div 
                  key={f.title} 
                  className={`rounded-2xl p-6 group cursor-pointer flex flex-col justify-between ${cardClass}`}
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          <div className="text-center max-w-2xl mx-auto mb-14 space-y-3">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${isLight ? "bg-emerald-50 text-emerald-800 border-emerald-200" : "bg-emerald-500/10 text-emerald-300 border-emerald-500/25"}`}>
              ...
              <span>Vessel Classes &amp; Port Infrastructure</span>
            </span>
            <h2 className={`text-2xl sm:text-3xl font-black ${isLight ? "text-slate-900" : "text-white"}`}>
              Bulk Carrier Fleet &amp; Draft Matrix
            </h2>
            <p className={`text-sm ${isLight ? "text-slate-600" : "text-slate-400"}`}>
              Dynamic matching between dry bulk carrier specifications and East Coast Indian port infrastructure.
            </p>
          </div>
      ...
      <section id="flow" className={`py-20 border-b ${isLight ? "bg-white border-slate-200" : "bg-[#030712] border-slate-800"}`}>
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          <div className="text-center max-w-2xl mx-auto mb-14 space-y-3">
            ...
            <h2 className={`text-2xl sm:text-3xl font-black ${isLight ? "text-slate-900" : "text-white"}`}>
              From Raw Market Signals to Executed Charter
            </h2>
            <p className={`text-sm ${isLight ? "text-slate-600" : "text-slate-400"}`}>
              An end-to-end intelligent pipeline transforming global maritime data into cost-optimized procurement.
            </p>
          </div>
          ...
          <div className="mt-14 rounded-3xl p-8 lg:p-10 flex flex-col sm:flex-row items-center justify-between gap-6 bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-700 shadow-xl text-white">
            <div className="space-y-1 text-center sm:text-left">
              <h3 className="text-xl sm:text-2xl font-black text-white">
                Ready to evaluate the full FreightSense platform?
              </h3>
              <p className="text-sm text-cyan-100 max-w-xl">
                Access all 6 intelligence modules, live AIS feeds, and interactive charter scenario engines immediately.
              </p>
            </div>
            <button 
              onClick={() => navigate("/login")} 
      ...
      <footer className={`py-12 ${isLight ? "bg-white text-slate-700" : "bg-[#030712] text-slate-300"}`}>
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2">
                {[
                  { label: "Freight Forecast", href: "/forecast" },
                  { label: "Vessel Recommendation", href: "/vessels" },
                  { label: "Port Intelligence", href: "/ports" },
                  { label: "Route Analysis", href: "/routes" },
                  { label: "Risk Sentinel", href: "/risk" },
                  { label: "Charter Planning", href: "/charter" }
                ].map(l => (
                  <li key={l.href}>
          ...
          <div className={`mt-10 pt-6 border-t flex flex-col sm:flex-row items-center justify-between gap-3 text-[11px] ${isLight ? "border-slate-200 text-slate-500" : "border-slate-800 text-slate-500"}`}>
            <span>© 2026 FreightSense — Smart India Hackathon Prototype. All data illustrative.</span>
            <button 
              onClick={toggleTheme} 
